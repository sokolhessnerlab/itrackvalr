...
```{r, view-b-df}
knitr::kable(
  head(b_df_hash[["CSN001"]], 20)
)
```

### Transform Behavioral Data `step_x`

```{r}
transform_step_1 <- function(df) {
  df %>%
    dplyr::mutate(
      # do something with the participant data frame and change step_1 field
    )
}

ls(b_df_hash) %>%
  purrr:::map(~ b_df_hash[[.x]]) %>%
  purrr::map(transform_step_1)
```
